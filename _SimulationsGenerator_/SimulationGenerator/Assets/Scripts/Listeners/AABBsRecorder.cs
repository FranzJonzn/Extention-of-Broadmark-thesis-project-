// SimulationGenerator - AABBsRecorder.cs (2019)
...
public class AABBsRecorder : SimulationListener {
    private FileStream fs;
    private BinaryWriter bw;
    private string fileName;

    public override void OnStart(Simulation simulation) {
        this.fileName = $"{simulation}.aabbs";

        string dir = Path.Combine(Simulation.OutputRoot, "Recordings_AABBs/");
...
//////////////
// EX JOBB ///////////////////////////////////////////////////////////////
    public override void OnStart(Simulation simulation, string outPutRoot)
    {
        this.fileName = $"{simulation}.aabbs";

        string dir = Path.Combine(outPutRoot, "Recordings_AABBs/");
        Directory.CreateDirectory(dir);
        this.fs = new FileStream(Path.Combine(dir, this.fileName), FileMode.OpenOrCreate);
        this.bw = new BinaryWriter(this.fs);
