...
namespace mn {
	...
	std::unordered_map<std::string, CudaDevice::KernelConfig>	
					CudaDevice::_kFuncTable;

	CudaDevice::KernelConfig::KernelConfig(KernelFunc f, cudaFuncCache cacheConfig, bool isWave) :
		func(f), cachePreference(cacheConfig), waveFashion(isWave) {
		cudaFuncGetAttributes(&attribs, f);
		maxOccBlockSize = evalOptimalBlockSize(attribs, cachePreference);
